...
#need to make sure timestamp is also called datetime for later allignment
OWASIS_BBB <- OWASIS_BBB %>% 
  rename(datetime = day)

Combined_SM_OWASIS <- inner_join(Combined_SM, OWASIS_BBB, by = "datetime")


#plot for the whole year comparing the two tools without averages
ggplot(Combined_SM_OWASIS) +
  geom_line(mapping = aes(x = datetime, y = SM1, color = "Tensio"), size = 0.3) +
  geom_line(mapping = aes(x = datetime, y = SWC_1_015, color = "Sentek015"), size = 0.3) +
  geom_line(mapping = aes(x = datetime, y = LAW_MS, color = "OWASIS"), size = 0.3) +
  labs (
    title = "Soil moisture measurement tool comparison",
    x = "Date",
    y = "Soil moisture",
  ) +
  scale_color_manual(values = c("Tensio" = "red", "Sentek015" = "blue", "OWASIS" = "cadetblue"))
